...
int helper(const string &s, int i, long long num, int sign) {
    ...
    // Clamp if overflow
    if (sign * num <= INT_MIN_VAL) return INT_MIN_VAL; //Underflow Case
    if (sign * num >= INT_MAX_VAL) return INT_MAX_VAL;  //Overflow Case
...
// Tme Complexity: O(n) since each character is processed once.

// Space Complexity: O(n) since the recursion stack grows up to n calls.i