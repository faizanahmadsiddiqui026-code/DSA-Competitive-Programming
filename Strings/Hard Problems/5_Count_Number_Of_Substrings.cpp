...
int atMostKDistinct(string s, int k) {
    ...
    // Iterate through string with right pointer from left side
    for (int right = 0; right < s.size(); right++) {
        // Add current character
        freq[s[right]]++;
        // Shrink window if distinct characters exceed k
        while (freq.size() > k) {  //Size decide by distinct key count in freq unordered map
            freq[s[left]]--;
        ...
        res += (right - left + 1);  //Here Right and Left means Right Pointer current index and Left Pointer current index
    }
    return res;
}
